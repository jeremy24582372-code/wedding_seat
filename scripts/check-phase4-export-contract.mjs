...
function getAnnotatedTables(model) {
  return [
...
function checkFloorPrintHtmlContract() {
  ...
  const model = buildWeddingFloorLayoutModel(floorSeatLabelState, options);
  const annotatedTables = getAnnotatedTables(model);
  const occupiedGuestIds = annotatedTables
    .flatMap(table => table.seats)
    .filter(seat => seat.guest)
    .map(seat => seat.guest.id);
  const annotationGuestIds = annotatedTables
    .flatMap(table => table.annotationRecords)
    .map(annotation => annotation.guestId);
  ...
  assert.equal(
    ...
    annotationGuestIds.length,
    'Total rendered connector count must equal annotation count'
  );
  assert.equal(
    connectorGuestIds.length,
    annotationGuestIds.length,
    'Rendered connector data-guest-id count must equal annotation count'
  );
  assert.deepEqual(
    sorted(connectorGuestIds),
    sorted(annotationGuestIds),
    'Rendered connector guest IDs must match annotations'
  );
  assert.ok(
