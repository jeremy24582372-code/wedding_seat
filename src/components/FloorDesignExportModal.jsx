...
const DEFAULT_PREVIEW_SETTINGS = {
  horizontal: 25,
  vertical: 14,
  guestNameFontScale: 1,
  showTableOccupancy: true,
};

export default function FloorDesignExportModal({ state, mode = 'pdf', onClose }) {
  const exportConfig = EXPORT_MODE[mode] ?? EXPORT_MODE.pdf;
  const [draftSettings, setDraftSettings] = useState(DEFAULT_PREVIEW_SETTINGS);
  const [preview, setPreview] = useState(null);
  const [isExporting, setIsExporting] = useState(false);
  const isStale = preview && (
    preview.settings.horizontal !== draftSettings.horizontal ||
    preview.settings.vertical !== draftSettings.vertical ||
    preview.settings.guestNameFontScale !== draftSettings.guestNameFontScale ||
    preview.settings.showTableOccupancy !== draftSettings.showTableOccupancy
  );
  ...
  const helperText = useMemo(() => {
    if (!preview) return '調整預覽設定後，請按「產生預覽」才會重新計算桌位。';
    if (isStale) return '預覽設定已變更，請重新產生預覽；下載與列印會等到預覽更新後才開放。';
    return '目前預覽已套用所有設定。桌子大小固定，桌位會以互動畫布中心向左右／上下展開。';
  }, [isStale, preview]);
  ...
  const handleGeneratePreview = () => {
    try {
      ...
      const artifact = buildFloorDesignSvgExport(state, {
        date,
        floorDesign: {
          minHorizontalTableGapMm: draftSettings.horizontal,
          minVerticalTableGapMm: draftSettings.vertical,
          guestNameFontScale: draftSettings.guestNameFontScale,
          showTableOccupancy: draftSettings.showTableOccupancy,
        },
      });
      setPreview({
        artifact,
  ...
  const handleExport = async () => {
    ...
    try {
      const options = {
        date: preview.date,
        floorDesign: {
          minHorizontalTableGapMm: preview.settings.horizontal,
          minVerticalTableGapMm: preview.settings.vertical,
          guestNameFontScale: preview.settings.guestNameFontScale,
          showTableOccupancy: preview.settings.showTableOccupancy,
        },
      };
        ...
        <div className="floor-export-modal__body">
          <aside className="floor-export-modal__settings" aria-label="桌次圖匯出設定">
            <label className="floor-export-modal__field" htmlFor="floor-preview-min-gap-x">
              <span>左右最小間距</span>
              <strong>{draftSettings.horizontal} mm</strong>
              <input
                ...
                max="40"
                step="1"
                value={draftSettings.horizontal}
                onInput={event => handleSettingChange('horizontal', Number(event.target.value))}
                onChange={event => handleSettingChange('horizontal', Number(event.target.value))}
              />
            </label>

            <label className="floor-export-modal__field" htmlFor="floor-preview-min-gap-y">
              <span>上下最小間距</span>
              <strong>{draftSettings.vertical} mm</strong>
              <input
                ...
                max="16"
                step="1"
                value={draftSettings.vertical}
                onInput={event => handleSettingChange('vertical', Number(event.target.value))}
                onChange={event => handleSettingChange('vertical', Number(event.target.value))}
              />
            </label>

            <label className="floor-export-modal__field" htmlFor="floor-preview-name-font-scale">
              <span>姓名字體大小</span>
              <strong>{Math.round(draftSettings.guestNameFontScale * 100)}%</strong>
              <input
                id="floor-preview-name-font-scale"
                type="range"
                min="0.7"
                max="1.4"
                step="0.05"
                value={draftSettings.guestNameFontScale}
                onInput={event => handleSettingChange('guestNameFontScale', Number(event.target.value))}
                onChange={event => handleSettingChange('guestNameFontScale', Number(event.target.value))}
              />
            </label>

            <label className="floor-export-modal__toggle" htmlFor="floor-preview-show-table-occupancy">
              <input
                id="floor-preview-show-table-occupancy"
                type="checkbox"
                checked={draftSettings.showTableOccupancy}
                onChange={event => handleSettingChange('showTableOccupancy', event.target.checked)}
              />
              <span>
                <strong>顯示各桌人數</strong>
                <small>取消勾選後，桌次名稱會置中。</small>
              </span>
            </label>
            ...
            {model && (
              <div className="floor-export-modal__metrics" aria-label="預覽解析結果">
                <span>
                  <small>設定左右間距</small>
                  <strong>{preview.settings.horizontal} mm</strong>
                </span>
                <span>
                  <small>實際左右間距</small>
                ...
                <span>
                  <small>設定上下間距</small>
                  <strong>{preview.settings.vertical} mm</strong>
                </span>
                <span>
                  <small>實際上下間距</small>
                ...
                <span>
                  <small>姓名字體大小</small>
                  <strong>{Math.round(preview.settings.guestNameFontScale * 100)}%</strong>
                </span>
                <span>
                  <small>桌次人數</small>
                  <strong>{preview.settings.showTableOccupancy ? '顯示' : '隱藏'}</strong>
                </span>
              </div>
            )}
          ...
          <section className="floor-export-modal__preview" aria-label="桌次圖預覽">
            {preview ? (
              ...
              <div className="floor-export-modal__empty">
                ...
                <span>設定預覽選項後按「產生預覽」，這裡才會顯示實際輸出。</span>
              </div>
            )}
          </section>
        </div>
